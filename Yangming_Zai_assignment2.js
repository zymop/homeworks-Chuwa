...
let returnedString = string.toLowerCase().trim().split(/[ -]+/).join(" ");
console.log("Q2: -----------------------------");
...
const getMergedObjectArray = (first, second) => {
  const map = new Map();
  first.forEach((ele) => map.set(ele.uuid, ele));
  second.forEach((obj) => {
    if (map.has(obj.uuid)) {
      map.get(obj.uuid).role = map.get(obj.uuid).role || obj.role;
      map.get(obj.uuid).name = map.get(obj.uuid).name || obj.name;
    } else {
      map.set(obj.uuid, {
        uuid: obj.uuid || null,
        role: obj.role || null,
  ...
  return Array.from(map.values()).sort((obj1, obj2) => obj1.uuid - obj2.uuid);
  /*
  const reducedObjectArray = [...first, ...second].reduce((acc, obj) => {
    const found = acc.find((ele) => ele.uuid == obj.uuid);
    const newObj = {
      uuid: obj.uuid || null,
      role: obj.role || null,
      name: obj.name || null,
...
//------------------------------------------------------------------------------------------
/* output:
[Running] node "c:\Users\Yangming Zai\Desktop\chuwa\homework\homework\Yangming_Zai_assignment2.js"
Q1: -----------------------------
[
...
[ { quantity: 3, price: 350 }, { quantity: 5, price: 400 } ]
3250
Q2: -----------------------------
perhaps the easiest to understand case for reduce is to return the sum of all the elements in an array
true
