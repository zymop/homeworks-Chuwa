...
const createRowHelper = (arr, type) => {
  console.log(arr);
  const row = createElementHelper("tr", "", "");
  const len = arr.length;
  const eles = arr.map((ele) => {
    return ele === arr[len - 1]
      ? createElementHelper(type, ele, "longCell")
      : createElementHelper(type, ele, "cell");
  });
  row.append(...eles);
  return row;
};
...
const createListHelper = (list, type) => {
  const li = createElementHelper(type, "", "ordered-list");
...
document.body.appendChild(createListHelper(list, "ol"));
document.body.appendChild(createListHelper(list, "ul"));
...
const createDropDownListHelper = (enter) => {
  ...
  const eles = enter.map((obj) => {
    ...
    option.setAttribute("value", obj.value); //option.value = obj.value;
    return option;
  });
